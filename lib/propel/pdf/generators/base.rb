  ...
  module Pdf
    module Generators
      class Base
        ...
        def generate
          Propel.logger.info "Generating PDF - #{generation_context.inspect}"
          ...
          Propel.logger.error "PDF generation failed: #{e.message}"
          Propel::Result.error(e.message, status: :unprocessable_entity)
        rescue => e
          Propel.logger.error "Unexpected error in PDF generation: #{e.message}"
          Propel.logger.error e.backtrace.join("\n")
          Propel::Result.error("PDF generation failed", status: :internal_server_error)
        ensure
          cleanup_temp_files
        end
        ...
        def create_temp_file(prefix: 'propel', suffix: '.png')
        ...
        def validate_options!
          if options[:page_size] && !VALID_PAGE_SIZES.include?(options[:page_size].to_sym)
            raise Propel::Errors::Error, "Invalid page size. Valid sizes are: #{VALID_PAGE_SIZES.join(', ')}"
          end
        end
        ...
        def cleanup_temp_files
          @temp_files.each do |temp_file|
            ...
            Propel.logger.warn "Failed to cleanup temp file: #{e.message}"
          end
        end
