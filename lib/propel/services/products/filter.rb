  ...
  module Services
    module Products
      class Filter
        ...
        def apply
          Propel.logger.info "Applying filters", context: filter_context
          ...
          Propel.logger.error "Filter error: #{e.message}"
          Propel::Result.error(e.message, status: :unprocessable_entity)
        rescue => e
          Propel.logger.error "Unexpected error in filter: #{e.message}"
          Propel.logger.error e.backtrace.join("\n")
          Propel::Result.error("Filtering failed", status: :internal_server_error)
        end
