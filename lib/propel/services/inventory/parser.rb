  ...
  module Services
    module Inventory
      class Parser
        ...
        def parse
          Propel.logger.info "Parsing inventory command: #{@input}"
          match = @input.match(ADJUSTMENT_REGEX)
          unless match
            Propel.logger.warn "Invalid command format: #{@input}"
          ...
          Propel.logger.info "Parsed command", context: { quantity: quantity, identifier: identifier }
          Result.success(data: { quantity: quantity, identifier: identifier })
        end

        def parse_bulk
          Propel.logger.info "Parsing bulk inventory commands"
          commands = @input.split(BULK_SEPARATOR).map(&:strip).reject(&:empty?)
          Propel.logger.info "Found #{commands.length} commands to process"

          results = commands.map do |cmd|
            parse_result = Parser.parse(cmd)
            unless parse_result.success?
              Propel.logger.warn "Invalid command in bulk operation: #{cmd}"
