  ...
  module Services
    module Inventory
      class Command
        ...
        def process
          Propel.logger.info "Processing #{@bulk ? 'bulk' : 'single'} inventory command"
          ...
          Propel.logger.error "Command processing failed: #{e.message}"
        ...
        private
          ...
          def process_single_command
            Propel.logger.info "Processing command: #{command}"
          ...
          def process_bulk_command
            Propel.logger.info "Processing bulk commands"
          ...
          def validate_configuration!
            raise Propel::Errors::ConfigurationError, "Variant finder must be provided" unless @variant_finder
            raise Propel::Errors::ConfigurationError, "Command cannot be empty" if command.nil? || command.empty?
          end
          ...
          def log_adjustment_result(adjustment_data, result)
            ...
            if result.success?
              Propel.logger.info "Adjustment succeeded", context: context
            else
              Propel.logger.warn "Adjustment failed", context: context.merge(error: result.error)
            end
          end
          ...
          def build_bulk_result(results)
            ...
            if failed.any?
              Propel.logger.warn "Some adjustments failed", context: {
                total: results.size,
