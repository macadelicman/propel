# lib/propel/inventory/adjuster.rb
module Propel
  module Inventory
    class Adjuster
      ...
      def self.process_command(command)
        parsed = Propel::Inventory::Parser.parse_command(command)
      ...
      def self.process_commands(commands)
        parsed = Propel::Inventory::Parser.parse_commands(commands)
        return [parsed[:error]] if parsed[:error]
        commands.map do |cmd|
          process_command(cmd)
        end
      ...
      private
        ...
        def self.find_variant(identifier)
          Propel::Models::Variant.find_by(variant_sku: identifier) ||
            Propel::Models::Variant.find_by(uuid: identifier) ||
            Propel::Models::Variant.joins(:product).find_by("propel_products.title ILIKE ?", "%#{identifier}%")
        end
    end
  end
end
