  ...
  module Services
    module Inventory
      class Adjuster
        ...
        def adjust
          Propel.logger.info "Adjusting inventory", context: adjustment_context
        ...
        rescue Propel::Errors::Error => e
          Propel.logger.error "Validation error: #{e.message}"
          Result.error(e.message, status: :unprocessable_entity)
        rescue => e
          Propel.logger.error "Adjustment failed: #{e.message}"
          Propel.logger.error e.backtrace.join("\n")
          Result.error("Error adjusting inventory: #{e.message}", status: :internal_server_error)
        end

        def adjust_by_identifier
          Propel.logger.info "Looking up variant by identifier: #{@identifier}"

          find_variant
          adjust
        rescue Propel::Errors::Error => e
          Propel.logger.error "Variant lookup failed: #{e.message}"
        ...
        private
          ...
          def validate_quantity!
            unless quantity.is_a?(Integer)
              raise Propel::Errors::Error, "Quantity must be an integer"
            end
          end

          def find_variant
            raise Propel::Errors::Error, "No variant finder provided" unless @variant_finder

            @variant = @variant_finder.call(@identifier)

            unless @variant
              raise Propel::Errors::Error, "Variant not found for identifier: #{@identifier}"
            end
          end
          ...
          def calculate_new_quantity
            ...
            if new_quantity.negative?
              Propel.logger.warn "Adjustment would result in negative inventory, setting to 0",
          ...
          def update_inventory(new_quantity)
            ...
            if variant.update_inventory_quantity(new_quantity)
              Propel.logger.info "Successfully updated inventory",
                               context: { new_quantity: new_quantity }
              true
            else
              raise Propel::Errors::Error, "Failed to update inventory"
            end
          end
