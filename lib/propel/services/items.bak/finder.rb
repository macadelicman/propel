  ...
  module Services
    module Products
      class Finder
        ...
        def find
          Propel.logger.info "Finding products", context: search_context
          ...
          Propel.logger.error "Product finder error: #{e.message}"
          Propel::Result.error(e.message, status: :unprocessable_entity)
        rescue => e
          Propel.logger.error "Unexpected error in product finder: #{e.message}"
          Propel.logger.error e.backtrace.join("\n")
          Propel::Result.error("Product finding failed", status: :internal_server_error)
        end
        ...
        private
          ...
          def validate_scope!
            unless scope
              raise Propel::Errors::Error, "Scope must be provided"
            end
          end
          ...
          def get_result_count(results)
            ...
            Propel.logger.warn "Could not determine result count: #{e.message}"
            0
          end
