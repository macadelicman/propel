  ...
  module Services
    module Items
      class Paginator
        ...
        def paginate
          Propel.logger.info "Paginating results", context: pagination_context
          ...
          Propel.logger.error "Pagination error: #{e.message}"
          Propel::Result.error(e.message, status: :unprocessable_entity)
        rescue => e
          Propel.logger.error "Unexpected error in paginator: #{e.message}"
          Propel.logger.error e.backtrace.join("\n")
          Propel::Result.error("Pagination failed", status: :internal_server_error)
        end
