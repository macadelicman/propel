    ...
    module Db
      class DatabaseOperationsHandler
        ...
        def self.handle_db_operation(model, query)
          ...
          Propel.logger.error("Error processing query: #{e.message}")
          raise e
        end
        ...
        def execute_operation
          case parsed_operation["operation_type"]
            ...
            when "search"
              ...
              Propel.logger.warn("Unsupported operation type: #{parsed_operation['operation_type']}")
              nil
          end
        end
        ...
        private
          ...
          def handle_create(attributes)
            ...
            Propel.logger.info("Creating #{model.name} with attributes: #{sanitized_attributes}")
          ...
          def handle_update(conditions, attributes)
            raise Propel::Errors::Error, "Update requires specific conditions" if conditions.nil? || conditions.empty?

            sanitized_conditions = sanitize_conditions(conditions)
            sanitized_attributes = sanitize_attributes(attributes)
            Propel.logger.info("Updating #{model.name} records with conditions: #{sanitized_conditions} and attributes: #{sanitized_attributes}")
            model.execute_update(sanitized_conditions, sanitized_attributes)
          end

          def handle_delete(conditions)
            raise Propel::Errors::Error, "Delete requires specific conditions" if conditions.nil? || conditions.empty?

            sanitized_conditions = sanitize_conditions(conditions)
            Propel.logger.info("Deleting #{model.name} records with conditions: #{sanitized_conditions}")
