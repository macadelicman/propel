    ...
    module Search
      class Similarity < Base
        ...
        def perform
          ...
          Propel.logger.info "Performing similarity search", context: search_context

          results = find_similar_products

          Propel.logger.info "Similarity search completed", context: result_context(results)
        ...
        private
          ...
          def validate_configuration!
            unless Propel.configuration.openai_api_key
              raise Propel::Errors::ConfigurationError, "OpenAI API key must be configured"
            end
          end
          ...
          def initialize_embeddings
            Langchain::LLM::OpenAI.new(
              api_key: options[:api_key] || Propel.configuration.openai_api_key,
              embeddings_model: options[:embeddings_model] || Propel.configuration.embeddings_model
            )
          end
          ...
          def search_limit
            options[:limit] ||
              Propel.configuration.default_search_limit ||
              DEFAULT_LIMIT
          end
          ...
          def search_context
            {
              ...
              limit: search_limit,
              embeddings_model: Propel.configuration.embeddings_model
            }
          end
