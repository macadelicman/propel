    ...
    module Search
      class Base
        ...
        protected
          ...
          def validate_scope!
            unless scope
              Propel.logger.error "No scope provided for search"
              raise Propel::Errors::ConfigurationError, "Search scope must be provided"
            end
          end
          ...
          def log_error(error, context = {})
            Propel.logger.error "Search error: #{error.message}"
            Propel.logger.error "Context: #{context.inspect}"
            Propel.logger.error error.backtrace.join("\n") if error.backtrace
          end
      end
    end
  end
